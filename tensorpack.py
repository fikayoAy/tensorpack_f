...
def get_application_path():
    """Get the path of the application, whether running as script or frozen exe"""
        ...
        if args.output:
            ...
            if hasattr(args, 'export_formats') and args.export_formats and args.export_formats != ['json']:
                try:
                    ...
                    if os.path.exists(convertto_path):
                        ...
                        try:
                            ...
                            # Import and execute convertto.py functions
                            ...
                            try:
                                ...
                                logging.info(f"Generated additional export formats using convertto.py")
                        ...
                        logging.warning(f"convertto.py not found at {convertto_path}, skipping additional exports")
...
def traverse_graph_command(args) -> int:
    ...
    try:
        ...
        # Use convertto.py for additional export formats if specified
        if hasattr(args, 'export_formats') and args.export_formats:
            try:
                import importlib.util
                import sys

                # Get the convertto.py module path
                ...
                if os.path.exists(convertto_path):
                    ...
                    try:
                        ...
                        # Load and execute convertto.py functions
                        spec = importlib.util.spec_from_file_location("convertto", convertto_path)
                        convertto = importlib.util.module_from_spec(spec)
                        ...
                        try:
                            spec.loader.exec_module(convertto)
                            print(f"Generated additional export formats using convertto.py")
                    ...
                    print(f"Warning: convertto.py not found at {convertto_path}, skipping additional exports")
...
def _explore_dataset_pathway(transformer, datasets, dataset_info, source_dataset, target_dataset, args):
    ...
    try:
        ...
        for entity in source_entities[:5]:  # Limit to top entities to avoid excessive processing
                    ...
                    if connection_strength > 0.05:  # Lowered to 0.05 to catch the 0.073 connections
                        ...
                        enriched_connection = {
                            ...
                            'entity_analysis': {
                                ...
                                'target_entities': target_entities[:10],
    ...
    for i, dataset in enumerate(datasets):
        try:
            ...
            if getattr(args, 'verbose', False) and entity_matches:
                for j, match in enumerate(entity_matches[:3]):  # Show first 3 matches
...
def _find_semantic_connections_between_datasets(source_data, target_data, source_info, target_info):
    ...
    try:
        ...
        if source_info.get('data_type') == 'json' and target_info.get('data_type') == 'json':
        ...
        if source_info.get('data_type') == 'excel' and target_info.get('data_type') == 'excel':
        ...
        elif source_info.get('data_type') == 'tabular' and target_info.get('data_type') == 'tabular':
        ...
        elif 'image' in source_info.get('data_type', '') or 'image' in target_info.get('data_type', ''):
...
def _find_tabular_connections(source_data, target_data, source_info, target_info):
    ...
    # Look for similar value ranges or distributions
    if source_data.ndim == 2 and target_data.ndim == 2:
        source_stats = {
            'mean': np.mean(source_data),
            'std': np.std(source_data),
            'min': np.min(source_data),
            'max': np.max(source_data)
        }
        target_stats = {
        ...
        mean_similarity = 1.0 / (1.0 + abs(source_stats['mean'] - target_stats['mean']))
        range_similarity = 1.0 / (1.0 + abs((source_stats['max'] - source_stats['min']) - 
                                           (target_stats['max'] - target_stats['min'])))
        
        if mean_similarity > 0.7 or range_similarity > 0.7:
            connections.append({
                'type': 'statistical_similarity',
                'mean_similarity': float(mean_similarity),
                'range_similarity': float(range_similarity),
                'description': 'Datasets show similar statistical distributions',
