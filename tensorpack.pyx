...
def get_numeric_df_columns(df) -> list:
    """Get list of numeric column names from DataFrame."""
    return [col for col in df.columns if np.issubdtype(df[col].dtype, np.number)]

def get_object_df_columns(df) -> list:
    """Get list of object/string column names from DataFrame."""
    return [col for col in df.columns if df[col].dtype == object]

# === Global helper metric functions (exposed for unit tests) ===

def get_numeric_columns(df):
    """Get numeric columns from a DataFrame in a Cython-friendly way.
    Returns a new DataFrame with only numeric columns."""
    numeric_data = []
    for col in df.columns:
        if np.issubdtype(df[col].dtype, np.number):
            numeric_data.append(df[col])
    if numeric_data:
    ...
    try:
        ...
        if file_ext == '.json':
            ...
            try:
                ...
                # Use numpy dtype objects which Cython understands
            ...
            try:
                ...
                # Filter numeric columns by checking dtype properties
                numeric_cols = []
                for col in df.columns:
                    if np.issubdtype(df[col].dtype, np.number):
            ...
            if data_type == 'image':
                ...
                for delimiter in [',', '\t']:
                    try:
                        ...
                        # Convert to numeric where possible and get numeric columns
                        numeric_data = []
                        for col in df.columns:
                            try:
                                series = pd.to_numeric(df[col], errors='coerce')
                                df[col] = series
                                if series.dtype.kind in 'iuf':  # integer, unsigned int, or float
                                    numeric_data.append(series)
                            except (ValueError, TypeError):
                                continue
                        
                        if numeric_data:
                            df = pd.concat(numeric_data, axis=1)
                            df = df.fillna(0)
                            result_array = df.values
                        else:
                            result_array = np.zeros((df.shape[0], 1), dtype=np.float64)
...
def _extract_contextual_info(metadata, search_entity):
    ...
    if metadata.get('is_tabular') or data_type == 'tabular':
        ...
        def count_numeric_columns(df):
            """Helper to count numeric columns in a DataFrame"""
            return sum(1 for col in df.columns if np.issubdtype(df[col].dtype, np.number))

        # Add column type analysis if dataframe is available
        if 'dataframe' in metadata:
            df = metadata['dataframe']
            context.update({
                'numeric_columns': count_numeric_columns(df),
                'text_columns': sum(1 for col in df.columns if df[col].dtype == np.dtype('O')),
