...
class LicenseManager:
    ...
    def __init__(self):
        ...
        try:
            # Assuming public_key.pem is in the same directory as this script, or in the root of the project
            key_path = Path(__file__).parent.parent / "public_key.pem"
            if not key_path.exists():
                # Fallback to workspace root if not found relative to the script
                key_path = Path.cwd() / "public_key.pem"

            with open(key_path, "rb") as f:
                self.public_key = f.read()
        except Exception as e:
            logger.error(f"Could not load public_key.pem: {e}. License verification will fail.")
