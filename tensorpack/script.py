...
def activate_license_command(args) -> int:
    ...
    try:
        ...
        license_key_or_file = args.license_key
        print(f"DEBUG: Input argument: {license_key_or_file}")
        print(f"DEBUG: Current working directory: {os.getcwd()}")
        
        # Try to resolve the file path more thoroughly
        file_path = os.path.abspath(license_key_or_file)
        print(f"DEBUG: Absolute path: {file_path}")
        print(f"DEBUG: File exists check: {os.path.exists(file_path)}")
        print(f"DEBUG: Is file check: {os.path.isfile(file_path)}")
        
        # Also check if it's in the current directory
        current_dir_path = os.path.join(os.getcwd(), license_key_or_file)
        print(f"DEBUG: Current dir path: {current_dir_path}")
        print(f"DEBUG: Current dir file exists: {os.path.exists(current_dir_path)}")
        
        if os.path.isfile(file_path) or os.path.isfile(current_dir_path):
            # It's a file path, read the JWT token from it
            actual_file_path = file_path if os.path.isfile(file_path) else current_dir_path
            print(f"DEBUG: Reading from file: {actual_file_path}")
            try:
                with open(actual_file_path, 'r', encoding='utf-8') as f:
                    jwt_token = f.read().strip()
                print(f"DEBUG: Token from file - length: {len(jwt_token)}, segments: {len(jwt_token.split('.'))}")
                print(f"DEBUG: Token first 50 chars: {jwt_token[:50]}...")
            except Exception as e:
                print(f"DEBUG: Error reading file: {e}")
                jwt_token = license_key_or_file  # Fallback to treating as token
        else:
            # It's already a JWT token string
            jwt_token = license_key_or_file
            print(f"DEBUG: Using direct token - length: {len(jwt_token)}, segments: {len(jwt_token.split('.'))}")
            
        success, message = license_manager.activate_license(jwt_token)
